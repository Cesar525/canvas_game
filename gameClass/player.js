class Player {
	constructor(name, level, thruster_selection, get_health, energy, m_damage, gunType, posx, posy, gun_speed, flag) {
		(this.position = {
			x: c.width / 2, //
	...
	shottingiinterval() {
		...
		if (keys.shotting.pressed) {
			...
			if (this.shotting_interval == 1) {
				gunsType(this.body.m_gun_type, this, this.position.x, this.position.y, this.body.m_name, this.body.m_flag);
			}
	...
	powerBombBar() {
		...
		ctx.fillRect(
			...
			5,
			60,
			200 - Math.round((this.body.powerBombEnergy / this.body.total_set_powerbombenergy) * 190)
		);
		...
		ctx.fillRect(
			...
			10,
			50,
			180 - Math.round((this.body.powerBombEnergy / this.body.total_set_powerbombenergy) * 180)
		);
		ctx.fillStyle = 'white';
		ctx.font = '30px Roboto Mono';
		ctx.strokeStyle = 'black';
